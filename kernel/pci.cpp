...
/** @brief devices[num_dievce] に情報を書き込み num_deivce をインクリメントする
 */
 ...
 */
Error ScanFunctions(uint8_t bus, uint8_t device, uint8_t function) {
...
Error ScanDevice(uint8_t bus, uint8_t device) {
  if (auto err = ScanFunctions(bus, device, 0)) {
    return err;
  }
  ...
  for (uint8_t function = 1; function < 8; ++function) {
    ...
    if (auto err = ScanFunctions(bus, device, function)) {
      return err;
    }
  }
  return Error::kSuccess;
